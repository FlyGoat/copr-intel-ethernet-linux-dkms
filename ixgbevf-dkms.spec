%global dkms_name ixgbevf
%global upstream_repo intel/ethernet-linux-ixgbevf
%global _disable_source_fetch 0

Name:           ixgbevf-dkms
Version:        5.3.36
Release:        1%{?dist}
Summary:        DKMS package for the Intel 10GbE PCI Express Virtual Function driver
...
%description
This package installs Intel's out-of-tree ixgbevf driver sources under
...
install -d "$dkms_src"
cp -a COPYING README src scripts pci.updates %{dkms_name}.7 "$dkms_src"/
cp -a SUMS "$dkms_src"/
...
MAKE[0]="make -C ${dkms_tree}/${PACKAGE_NAME}/${PACKAGE_VERSION}/build/src KSRC=${kernel_source_dir} BUILD_KERNEL=${kernelver}"
CLEAN="make -C ${dkms_tree}/${PACKAGE_NAME}/${PACKAGE_VERSION}/build/src KSRC=${kernel_source_dir} BUILD_KERNEL=${kernelver} clean"
EOF
...
%changelog
* Thu Jun 25 2026 FlyGoat <flygoat@users.noreply.github.com> - 5.3.36-1
- Initial DKMS package for Intel ixgbevf.
